...
def get_args_parser():
    ...
    parser.add_argument('--world_size', default=1, type=int,help='number of distributed pơ/esses')
...
def main(args):
    ...
    if (args.model == "optimize_model"):
        model = OptimizedConvNeXtMobileViT(num_classes=5)
        if args.finetune:
    ...
    elif (args.model == "conv_vit_base"):
        model = ConvNeXtMobileViTBase(num_classes=5)
        if args.finetune:
                checkpoint = torch.load(args.finetune, map_location=args.device)
                print("Load pre-trained checkpoint from: %s" % args.finetune)
                # print("key: ",checkpoint.keys())
        ...
        if args.finetune:
                ...
                if 'state_dict' in checkpoint.keys():
                    ...
                    checkpoint_model = checkpoint
                model.load_state_dict(checkpoint_model, strict=False)
    elif (args.model == "mobilevit-x-small"):
        model = MobileViTForImageClassification.from_pretrained("apple/mobilevit-x-small", num_labels=5,ignore_mismatched_sizes=True)
            ...
            if args.finetune:
                ...
                if 'state_dict' in checkpoint.keys():
                    ...
                    checkpoint_model = checkpoint
                model.load_state_dict(checkpoint_model, strict=False)
            # checkpoint = torch.load(args.finetune, map_location=device)  # Load trọng số
            # model.load_state_dict(checkpoint["model"])
    elif (args.model == "ConvSwin"):
            # model = ConvNeXt_Swin(num_classes=5)
            model = arch_functions["convnext_swin_base_34"](num_classes=5)
            if args.finetune:
                checkpoint = torch.load(args.finetune, map_location=args.device)
                print("Load pre-trained checkpoint from: %s" % args.finetune)
                # print("key: ",checkpoint.keys())
