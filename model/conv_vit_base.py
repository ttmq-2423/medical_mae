...
class ConvNeXtMobileViTBase(nn.Module):
    def __init__(self, num_classes=5, pretrained=True):
        super().__init__()
        self.backbone = timm.create_model("convnext_base", features_only=True, pretrained=False)
        # Stage 2 (feats[2]) has output channels = 384
        self.mobilevit = MobileViTBlock(dim=512, ffn_dim=768, n_transformer_blocks=2)
